...
# FUNGSI: Mengunduh skrip instalasi utama dari GitHub Anda.
# Pengembang: Anjas Amar Pradana / JTSI

set -e
GITHUB_RAW_URL="https://raw.githubusercontent.com/jtsi-project/AiProb/main/installer.sh"
...
echo "[1/2] Mengunduh skrip instalasi utama ('installer.sh') dari GitHub..."
if command -v curl &> /dev/null; then
    curl -sSL -o installer.sh "$GITHUB_RAW_URL"
elif command -v wget &> /dev/null; then
    wget -q -O installer.sh "$GITHUB_RAW_URL"
else
...
if [ ! -f "installer.sh" ]; then
    echo "ERROR: Pengunduhan gagal. Cek koneksi atau URL GitHub."
    exit 1
fi

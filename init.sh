#!/bin/bash
# --- AiProb v7.1 (Installer Interaktif & Cross-Env FINAL) ---
# Brand: JTSI (JAS TECH SYSTEM INSTRUMENT)
# Pengembang: Anjas Amar Pradana
#
# PERBAIKAN: Menghapus dependensi 'platform' (modul built-in) dan menambahkan
#            pemeriksaan akses ROOT untuk menjamin kompatibilitas cross-env.

# --- KONFIGURASI PROYEK ---
VENV_NAME=".venv_aiprob"
PYTHON_BIN="python3"
GITHUB_REPO_PATH="AnjasAmarPradana/AiProb" 
...
# --- Header ---
echo "================================================="
echo "== AiProb v7.1: Installer & Self-Checker Final =="
echo "================================================="
echo ""

# --- FASE 1.5: SHELL & ROOT CHECK (Cross-Env Safety) ---
echo "[CHECK: Keamanan Lingkungan]"

if [ -z "$BASH_VERSION" ]; then
    echo "⚠️ PERINGATAN KRITIS: Skrip ini harus dijalankan menggunakan BASH (bukan SH)!"
    echo "  -> Silakan ulangi dengan: bash init.sh atau ./init.sh"
fi

# Pengecekan ROOT (Hanya penting di Debian/Linux standar)
if [ "$(id -u)" = "0" ] && [ -z "$PREFIX" ]; then
    echo "🚨 AKSES ROOT TERDETEKSI! (Lingkungan Debian/Linux standar)"
    echo "  -> Program ini HARUS dijalankan sebagai USER BIASA (NON-ROOT)!"
    echo "  -> Kami akan menggunakan 'sudo' hanya untuk instalasi paket sistem."
    echo "  -> Jika ini Termux, Anda AMAN. Jika bukan, jalankan kembali sebagai user biasa."
else
    echo "✅ Akses Non-Root terdeteksi. Sesuai untuk lingkungan Termux atau user Debian."
fi
echo ""

# --- FASE 1: PERINGATAN & CLEARING LAMA ---
echo "[SETUP] Menganalisis direktori..."
if [ -f "app.py" ] || [ -f "jtsi_aiprob.db" ] || [ -d "$VENV_NAME" ]; then
    echo "  -> File instalasi lama terdeteksi (app.py, jtsi_aiprob.db, atau $VENV_NAME)."
    read -p "  -> Apakah Anda ingin MENGHAPUS file lama dan memulai setup baru? [Y/n] " PERSETUJUAN
...
read -p "Apakah Anda setuju untuk memulai instalasi? [Y/n] " PERSETUJUAN
if [ "$PERSETUJUAN" != "y" ] && [ "$PERSETUJUAN" != "Y" ] && [ "$PERSETUJUAN" != "" ]; then
    echo "Instalasi dibatalkan oleh pengguna."
    exit 0
fi
...
echo ""
echo "--- Memulai Pemeriksaan Sistem ---"
echo ""

# --- FASE 2: SELF-CHECK KONDISI SISTEM ---
echo "[CHECK 1/3] Deteksi Lingkungan..."
if [ -n "$PREFIX" ]; then
    echo "  -> Lingkungan Termux terdeteksi. Menggunakan 'pkg'."
    INSTALL_CMD="pkg install -y"
    SYS_DEPS="python python-pip build-essential"
    PYTHON_BIN="python"
    PKG_UPDATE="pkg update -y"
else
    echo "  -> Lingkungan Debian/Ubuntu terdeteksi. Menggunakan 'sudo apt-get'."
    INSTALL_CMD="sudo apt-get install -y"
    SYS_DEPS="python3 python3-venv python3-pip build-essential"
    PYTHON_BIN="python3"
    PKG_UPDATE="sudo apt-get update -y"
fi

echo "[CHECK 2/3] Memastikan Tools Dasar ('python', 'venv')..."
if ! command -v $PYTHON_BIN &> /dev/null
then
    echo "  -> $PYTHON_BIN tidak ditemukan. Memerlukan instalasi."
    echo "  -> Menginstal dependensi sistem: $SYS_DEPS"
    $PKG_UPDATE
    $INSTALL_CMD $SYS_DEPS || { echo "ERROR: Gagal menginstal dependensi sistem."; exit 1; }
else 
    echo "  -> $PYTHON_BIN ditemukan."
fi

# Pengecekan venv (untuk memastikan modul venv ada)
if ! $PYTHON_BIN -m venv --help &> /dev/null
then
    echo "  -> PERINGATAN: Modul 'venv' tidak tersedia. Mencoba menginstal."
    if [ -n "$PREFIX" ]; then
        $INSTALL_CMD python-pip python
    else
        $INSTALL_CMD python3-venv || { echo "ERROR: Gagal menginstal python3-venv!"; exit 1; }
    fi
fi

echo "[CHECK 3/3] Menyiapkan Lingkungan Virtual..."
$PYTHON_BIN -m venv $VENV_NAME || { echo "ERROR: Gagal membuat lingkungan virtual $VENV_NAME!"; exit 1; }

# Menggunakan operator titik (.) untuk aktivasi yang lebih kompatibel
. $VENV_NAME/bin/activate
echo "  -> Lingkungan virtual diaktifkan: $VENV_NAME/bin/activate"

# --- FASE 3: INSTALASI DEPENDENSI PYTHON & FILE PROYEK ---
echo ""
echo "--- FASE INSTALASI PROYEK ---"
echo "[TAHAP 1/4] Membuat File Dependensi requirements.txt (FIXED)..."
# MENGHAPUS 'platform' dari daftar requirements
...
echo "  -> Menginstal dependensi Python ke $VENV_NAME..."
pip install -r requirements.txt || { echo "ERROR: Gagal menginstal dependensi Python! Periksa koneksi internet."; deactivate; exit 1; }

echo ""
echo "[TAHAP 2/4] Membuat Struktur Direktori..."
mkdir -p templates
mkdir -p static
echo "  -> Direktori 'templates/' dan 'static/' dibuat."

# --- TAHAP 3/4: Membuat app.py (FULL CODE) ---
echo "[TAHAP 3/4] Membuat app.py (dengan Logic Self-Update & Live Log)..."
cat > app.py <<PYTHON_CODE_APP
# [KODE app.py LENGKAP - AiProb v7.1 - JTSI]
import sqlite3
import sys
import datetime
import os
...
# --- KONFIGURASI SISTEM (HARDCODED & LEGAL) ---
DB_NAME = 'jtsi_aiprob.db'
AI_BRAND = "JTSI (JAS TECH SYSTEM INSTRUMENT)"
AI_DEVELOPER = "Anjas Amar Pradana"
AI_NAME_DEFAULT = "AiProb"
MIN_PROBABILITY_SCORE = 75
STARTING_SCORE = 10
CURRENT_VERSION = "v7.1-release"
# URL ke file version.ini di root main branch GitHub Anda
...
class QueueHandler(logging.Handler):
    """Menangkap log dan memasukkannya ke dalam queue untuk Live Log."""
    ...
    try:
        with app.app_context():
            ...
            CREATE TABLE IF NOT EXISTS Users (
                ...
                ai_callsign TEXT DEFAULT '{AI_NAME_DEFAULT}'
            )
            ''')
            cursor.execute('''
...
def setup():
    ...
    if request.method == 'POST':
        ...
        if not admin_user or not admin_pass or not api_key:
            ...
            return redirect(url_for('setup'))
        if not inisialisasi_database():
            flash("Gagal inisialisasi database!", "danger")
            return redirect(url_for('setup'))
        hashed_pass = hash_password(admin_pass)
        try:
            execute_db(
                "INSERT INTO Users (username, hashed_password, role, ai_callsign) VALUES (?, ?, 'admin', ?)",
                (admin_user, hashed_pass, f"{AI_NAME_DEFAULT} (Admin)")
            )
            ...
            return redirect(url_for('setup'))
    return render_template('setup.html', brand=AI_BRAND, dev=AI_DEVELOPER)
        ...
        if user:
            ...
            try:
                execute_db(
                    ...
                    (username, hashed_pass, AI_NAME_DEFAULT)
                )
...
def dashboard():
    ...
    needs_update, latest_version, release_stage = check_for_updates()
    ...
    if user_data['role'] == 'admin':
        return render_template('admin_dashboard.html', user=user_data, settings=settings, brand=AI_BRAND, dev=AI_DEVELOPER)
    else:
        return render_template('user_dashboard.html', user=user_data, settings=settings, brand=AI_BRAND, dev=AI_DEVELOPER)
...
def admin_set_settings():
    ...
    if 'new_scope' in data:
        ...
        if new_scope in ['private', 'global']:
            execute_db("UPDATE Settings SET setting_va
